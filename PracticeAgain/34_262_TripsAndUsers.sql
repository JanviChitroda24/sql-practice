...
-- Any time a single row references the same lookup table through two different foreign keys. 
--     Common examples: 
--         trips (client/driver), 
--         messages (sender/receiver), 
--         transfers (from_account/to_account), 
--         games (home_team/away_team).
--     Each foreign key needs its own JOIN to the same table with its own alias.

-- Mental shortcut: 
--     Count the foreign keys pointing to the same table. That's how many times you join it.

-- Two JOINs 
--     = "keep this trip only if the client is unbanned AND the driver is unbanned" 
--     — one scan, one filter, done.
-- UNION/INTERSECT 
--     = split the problem into two halves, solve each separately, then recombine 

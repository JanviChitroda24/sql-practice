...
customer_mv AS (
    SELECT visited_on, 
        ...
        ROW_NUMBER() OVER(ORDER BY visited_on) AS day_no
    FROM customer_visit
)
...
-- Before submitting any window frame, mentally recite: ROWS BETWEEN [smaller] AND [larger] — always left-to-right in time.
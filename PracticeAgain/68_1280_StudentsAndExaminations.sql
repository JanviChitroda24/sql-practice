...
-- WITH exam_counts AS (
--     SELECT s.subject_name, e.student_id, COUNT(e.student_id) AS attended_exams
--     FROM Subjects s LEFT JOIN  Examinations e
--         ON s.subject_name = e.subject_name
--     GROUP BY s.subject_name, e.student_id
-- )
-- SELECT s.student_id, s.student_name, e.subject_name, COALESCE(e.attended_exams,0) AS attended_exams
-- FROM Students s LEFT JOIN exam_counts e 
...
FROM student_subject s LEFT JOIN exam_counts e 

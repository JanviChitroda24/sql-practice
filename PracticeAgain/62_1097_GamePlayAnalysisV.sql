...
SELECT first_login AS install_dt, 
    ...
    ROUND(
        (COUNT(
                CASE 
                    WHEN DATEDIFF(next_login, first_login) = 1
                    THEN 1
                END
            )/COUNT(player_id)
        ),2) AS Day1_retention

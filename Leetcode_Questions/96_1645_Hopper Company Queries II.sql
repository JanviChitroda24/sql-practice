...
-- https://leetcode.ca/all/1645.html
...
-- Write an SQL query to report the percentage of working drivers (working_percentage) for each month of 2020 where:Programming
...
driver_month AS (
    SELECT mn.mon, COUNT(d.driver_id) AS active_drivers
    FROM month_no mn LEFT JOIN Drivers d 
        ON  ( YEAR(d.join_date)<2020 OR 
            (YEAR(d.join_date)=2020 AND MONTH(d.join_date) <= mn.mon) )
    GROUP BY mn.mon
),
accepted_rides AS (
    SELECT MONTH(r.requested_at) mon, COUNT(DISTINCT ar.driver_id) AS accepted_drivers
    FROM Rides r JOIN AcceptedRides ar 
        ON r.ride_id = ar.ride_id
    WHERE YEAR(r.requested_at)=2020
    GROUP BY MONTH(r.requested_at)
),
mon_accepted_rides AS (
    SELECT mn.mon, ar.accepted_drivers 
    FROM month_no mn LEFT JOIN accepted_rides ar
        ON mn.mon = ar.mon
)
SELECT d.mon AS month, ROUND( COALESCE( a.accepted_drivers/NULLIF(d.active_drivers,0) , 0)*100 ,2) AS working_percentage
FROM driver_month d JOIN mon_accepted_rides a
    ON d.mon = a.mon
ORDER BY d.mon;
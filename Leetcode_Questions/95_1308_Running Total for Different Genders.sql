...
-- Write an SQL query to find the total score for each gender at each day.Programming
...
-- Good question. There's a key difference:

-- **`SUM(score_points) OVER(PARTITION BY gender)`** — no ORDER BY. This sums ALL rows in the partition. Every row gets the same total.

-- | gender | day | score_points | total |
-- |--------|-----|-------------|-------|
-- | F | 2019-12-30 | 17 | 80 |
-- | F | 2019-12-31 | 23 | 80 |
-- | F | 2020-01-01 | 17 | 80 |
...
-- **`SUM(score_points) OVER(PARTITION BY gender ORDER BY day)`** — with ORDER BY. The default frame becomes `ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW`. This creates a running/cumulative sum.
...
-- **The rule:** Adding `ORDER BY` inside `OVER()` automatically changes the default frame to cumulative. That's why you don't need to write `ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW` explicitly — it's already implied.

"""
1596 - Most Frequently Ordered Products for Each Customer
https://leetcode.ca/all/1596.html
...
prod_count_by_customer AS (
    SELECT customer_id,  product_id,    
    DENSE_RANK() OVER(PARTITION BY customer_id ORDER BY prod_count DESC) AS rnk
    FROM product_count
)
SELECT pc.customer_id,  pc.product_id, p.product_name
FROM prod_count_by_customer pc JOIN Products p
